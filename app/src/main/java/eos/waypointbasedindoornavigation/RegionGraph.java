...
class RegionGraph {
    ...
    List<eos.waypointbasedindoornavigation.Region> getRegionPath(
            String sourceRegion, String destinationRegion){

        Queue<Region> queue = new LinkedList<>();
        HashMap<String, eos.waypointbasedindoornavigation.Region> path = new HashMap<>();
        ...
        while(!queue.isEmpty()){
            ...
            eos.waypointbasedindoornavigation.Region regionNode = queue.remove();
